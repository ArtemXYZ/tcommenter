...
class Tcommenter:
    ...
    def _reader(self, sql: str | TextClause, **params: str | int | list) -> list[tuple]:
        ...
        * Описание механики:
            Метод основан на работе библиотеки "SQLAlchemy" ("execute()"). Опциональная передача параметров, позволяет
            сделать  self._reader() универсальным. Механизм передачи через multyparams в conn.execute(sql, multyparams))
            обеспечивает защиту от SQL-инъекций.
            В случае возникновения ошибки при осуществлении запроса, вызывается исключение RuntimeError с подробностями
            (SQLAlchemyError).
            ...
            * Пример вызова:
                ...
                params = self._reader(sql, params)
